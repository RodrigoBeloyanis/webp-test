...
if(getenv('sandbox')){ $qualidade = 100; } else { $qualidade = 80; }

$_GET['src'] 	= $_GET['img'];
$_GET['size']	= $_GET['w'].'x'.$_GET['h'];

if(!empty($_GET['l'])){ $_GET['src'] = $_GET['l'].'/'.$_GET['src']; }
...
define('JPEG_QUALITY',          $qualidade);    // Quality of generated JPEGs (0 - 100; 100 being best)
...
$trim = isset($_GET['trim']) ? max(0, min(1, $_GET['trim'])) : 1;
...
if (!file_exists($file_temp)) {
    ...
    $w = ($w != '') ? floor(max(8, min(1920, $w))) : '';
    $h = ($h != '') ? floor(max(8, min(1920, $h))) : '';
    if (strstr($size, '<')) {
        $h = $w;
        $crop = 0;
        $trim = 1;
